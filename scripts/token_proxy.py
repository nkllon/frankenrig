...
def persisted_request_metadata(headers: dict[str, str], allowlist: list[str]) -> dict[str, str]:
    # Strict allowlist: only persist headers that exactly match the configured allowlist.
    # Never persist common secret-bearing headers even if present in allowlist.
    forbidden_substrings = ("authorization", "proxy-authorization", "cookie", "set-cookie")
    allow_set = {key.lower() for key in allowlist}
    output: dict[str, str] = {}
    for key, value in headers.items():
        ...
        if any(sub in k for sub in forbidden_substrings):
            # Explicitly skip secret-bearing headers.
            continue
        if isinstance(value, str) and value.strip().lower().startswith("bearer "):
            # Do not persist bearer tokens or token-like values.
            continue
        output[k] = value
    return output
...
def readiness_status(config: dict[str, Any]) -> dict[str, Any]:
    checks: dict[str, Any] = {}
    ready = True
    reasons: list[str] = []
    ...
    try:
        ...
        ready = False
        reasons.append("config_resolution_error")

    try:
        storage = ensure_storage(config)
        db_ok = storage["db"].exists()
        raw_parent_ok = storage["raw_events"].parent.exists()
        checks["storage_db_exists"] = db_ok
        checks["storage_raw_parent_exists"] = raw_parent_ok
        if not db_ok or not raw_parent_ok:
            ready = False
            reasons.append("storage_not_ready")
    except Exception as exc:  # noqa: BLE001
        checks["storage_error"] = type(exc).__name__
        ready = False
        reasons.append("storage_init_failed")

    # Verify telemetry sink write ability (try a short-lived insert/delete transaction).
    try:
        def _telemetry_write_test() -> bool:
            storage = storage_paths(config)
            conn = sqlite3.connect(storage["db"])
            try:
                cur = conn.execute(
                    "INSERT INTO events_raw(event_id,timestamp_utc,correlation_id,raw_json) VALUES(?,?,?,?)",
                    ("__telemetry_write_test__", utc_now_iso(), "__telemetry_write_test__", "{}"),
                )
                conn.commit()
                conn.execute("DELETE FROM events_raw WHERE event_id = ?", ("__telemetry_write_test__",))
                conn.commit()
                return True
            finally:
                conn.close()

        telemetry_ok = _telemetry_write_test()
    except Exception:
        telemetry_ok = False
    checks["telemetry_write_ok"] = telemetry_ok
    if not telemetry_ok:
        ready = False
        reasons.append("telemetry_write_failure")
    pricing_ok = False
    coverage_ok = False
    try:
        catalog = pricing_catalog(config)
        pricing_ok = bool(catalog.get("pricing_version")) and isinstance(catalog.get("entries"), list)
    except Exception:
        pricing_ok = False
    checks["pricing_catalog_ready"] = pricing_ok
    if not pricing_ok:
        ready = False
        reasons.append("pricing_catalog_invalid")

    try:
        matrix = coverage_matrix(config)
        coverage_ok = isinstance(matrix.get("traffic_classes"), list) and len(matrix.get("traffic_classes", [])) > 0
    except Exception:
        coverage_ok = False
    checks["coverage_matrix_ready"] = coverage_ok
    if not coverage_ok:
        ready = False
        reasons.append("coverage_matrix_invalid")

    return {"ready": ready, "checks": checks, "reasons": sorted(set(reasons))}
...
def persist_event_with_reliability(config: dict[str, Any], event: dict[str, Any], fail_mode: str) -> dict[str, Any]:
    try:
        append_raw_event(config, event)
        return {"persisted": True, "sink_degraded": False, "error": None}
    except Exception as exc:  # noqa: BLE001
        if fail_mode == "fail_closed":
            raise
        return {"persisted": False, "sink_degraded": True, "error": type(exc).__name__}
...
def coverage_report(config: dict[str, Any]) -> dict[str, Any]:
    ...
    proofs_path_str = coverage_cfg.get("proofs_path")
    proofs: dict[str, Any] = {}
    if proofs_path_str:
        try:
            proofs = load_json(resolve_path(str(proofs_path_str)))
        except Exception:
            proofs = {}
    conn = sqlite3.connect(storage_paths(config)["db"])
    conn.row_factory = sqlite3.Row
        ...
        validations.append(
            {
                ...
                "proofs": {"positive_proof": pos_ok, "negative_proof": neg_ok},
            }
        )
...
def scan_raw_events_for_secrets(config: dict[str, Any]) -> dict[str, Any]:
    """
    Scan raw events stored in the DB for secret-bearing headers or values.
    Returns a dict with findings list. This is a deterministic proof step that
    can be used to assert secrets are not persisted.
    """
    ...
    conn.row_factory = sqlite3.Row
    try:
        rows = conn.execute("SELECT event_id, raw_json FROM events_raw").fetchall()
        for row in rows:
            raw_text = str(row["raw_json"] or "")
            event_id = row["event_id"]
            # Look for common secret indicators in persisted JSON.
            if '"authorization"' in raw_text.lower() or "bearer " in raw_text.lower() or '"cookie"' in raw_text.lower():
                findings.append({"event_id": event_id, "snippet": raw_text[:240]})
    finally:
        conn.close()
    return {"findings": findings, "count": len(findings)}


def cmd_prove_secrets(args: argparse.Namespace) -> int:
    config = load_runtime_config(args.config)
    result = scan_raw_events_for_secrets(config)
...
class TokenProxyHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path == "/ready":
            ...
            readiness = readiness_status(config)
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        metadata_allowlist = ((config.get("security") or {}).get("persisted_request_header_allowlist") or [])
        ...
        event = {
            ...
            "persisted_request_metadata": persisted_request_metadata({k: v for k, v in self.headers.items()}, metadata_allowlist),
        }
        event["dedupe_fingerprint"] = dedupe_fingerprint(event)

        sink_result: dict[str, Any] = {"persisted": True, "sink_degraded": False, "error": None}
        try:
            sink_result = persist_event_with_reliability(config, event, fail_mode)
        except Exception as exc:  # noqa: BLE001
            if status == "success":
                self._json_response(
                    503,
                    {
                        "ok": False,
                        "error": "telemetry_sink_failure",
                        "mode": "fail_closed",
                        "provider": provider,
                        "model": model,
                        "correlation_id": correlation_id,
                        "sink_error": type(exc).__name__,
                    },
                )
                return
            raise
        ...
        response_payload["proxy_telemetry"] = {
            ...
            "estimated_cost": estimated_cost,
            "pricing_version": pricing_version,
            "telemetry_sink_degraded": bool(sink_result.get("sink_degraded")),
        }
...
def main() -> int:
    ...
    p_prove = sub.add_parser("prove-secrets", help="Scan raw events for persisted secrets.")
    p_prove.set_defaults(func=cmd_prove_secrets)
